import numpy as np
from scipy.stats import poisson
import math
from functools import lru_cache

# --- Base Test Case Parameters (from Section 5.1) ---

# Economic parameters [cite: 411, 415]
P_PRICE = 100.0     # p, price of the product
C_HOLDING = 1.0     # c_h, holding cost per day
C_PRODUCT = 70.0    # c_p, cost of the product
C_SHIPPING = 10.0   # c_s, shipping cost of an online order
C_PENALTY = 10.0    # c_l, penalty of a lost sale (unfulfilled online order)

# Time parameters [cite: 410]
R_PERIOD_DAYS = 7   # R, review period (sub-periods)
L_LEAD_TIME_DAYS = 2 # L, fixed lead time

# Demand parameters [cite: 414]
# Demand is Poisson distributed, truncated. We'll use a practical upper limit
# for summation in expected value calculations.
MAX_DEMAND_STORE = 30 # Practical upper bound for D_j

# --- Store Class ---
class Store:
    """Represents a single retail store."""
    def __init__(self, id, type, mu_demand, initial_inventory):
        self.id = id
        self.type = type
        self.mu_demand = mu_demand  # mu_j, average in-store demand per day [cite: 414]
        self.inventory = initial_inventory # I_j
        self.outstanding_order = 0 # Q_j
    
    def __repr__(self):
        return f"Store {self.id} ({self.type}): Inv={self.inventory}, Q={self.outstanding_order}"

# --- Helper Functions for Myopic Policies ---

@lru_cache(maxsize=None)
def calculate_expected_contribution_next_day(inventory, mu_demand):
    """
    Calculates the simplified expected contribution (profit) from in-store sales
    for the *next day* (myopic view). This is the core of the MFP.
    Based on Eq. 5.1 [cite: 251] and used in Eq. 11[cite: 361].
    
    EC_j = p * (Expected_Sales) - c_h * I_j
    We ignore ordering costs (term 5.3) as we are evaluating a state[cite: 361].
    """
    if inventory <= 0:
        return 0.0
        
    expected_sales = 0.0
    # Sum over all possible demands d_j [cite: 251]
    # We use poisson.pmf for P_j(d_j) [cite: 414]
    for d in range(MAX_DEMAND_STORE + 1):
        prob = poisson.pmf(d, mu_demand)
        sales = min(inventory, d)
        expected_sales += prob * sales
        
    revenue = P_PRICE * expected_sales
    # Holding cost is determined at the beginning of the sub-period [cite: 254]
    holding_cost = C_HOLDING * inventory
    
    # This is EC_j(I_j, 0) as used in Eq. 11 [cite: 361]
    return revenue - holding_cost

# --- Policy 1: Myopic Fulfilment Policy (MFP) ---

def mfp_fulfillment_policy(stores, F_online_orders):
    """
    Allocates online orders based on the Myopic Fulfilment Policy (MFP).
    This implements the greedy logic from  to solve Algorithm 2.
    
    It assigns orders one-by-one to the store with the highest
    marginal gain, until the gain is no longer positive.
    
    Marginal Gain_j = (p - c_s) + c_l + (EC_j(I-1) - EC_j(I))
    We fulfill if Gain_j > 0.
    """
    
    # Create a copy of inventories to track allocations
    # This is I_j' (inventory after in-store demand) [cite: 187, 220]
    current_inventories = {s.id: s.inventory for s in stores}
    allocations = {s.id: 0 for s in stores}
    
    for _ in range(F_online_orders):
        best_store_id = None
        best_marginal_gain = -float('inf')
        
        for s in stores:
            inv = current_inventories[s.id]
            
            # Check if store has inventory left to assign
            if inv > 0:
                # Calculate marginal opportunity cost: EC_j(I-1) - EC_j(I) [cite: 361]
                ec_with_fulfill = calculate_expected_contribution_next_day(inv - 1, s.mu_demand)
                ec_without_fulfill = calculate_expected_contribution_next_day(inv, s.mu_demand)
                
                marginal_oc = ec_with_fulfill - ec_without_fulfill
                
                # Gain = Profit from online sale + Saved penalty + Marginal OC
                # This corresponds to maximizing Eq. 12 [cite: 365] greedily
                gain = (P_PRICE - C_SHIPPING) + C_PENALTY + marginal_oc
                
                if gain > best_marginal_gain:
                    best_marginal_gain = gain
                    best_store_id = s.id
        
        # Fulfill if the best gain is positive [cite: 368]
        if best_marginal_gain > 0:
            allocations[best_store_id] += 1
            current_inventories[best_store_id] -= 1
        else:
            # No store provides a positive gain, stop fulfilling
            break
            
    return allocations

# --- Policy 2: Threshold-based Myopic Fulfilment Policy (TMFP) ---

@lru_cache(maxsize=None)
def get_days_until_replenishment(t, L, R):
    """
    Calculates the number of days from sub-period t until the next
    replenishment arrives. Based on description in[cite: 390, 395].
    Replenishment arrives at the *start* of day L+1 (t=3)[cite: 410].
    """
    # t is 1-indexed (Day 1 to 7)
    if t <= L:
        # Need to cover demand for days t, t+1, ... L
        # t=1: L-1+1 = 2 (days 1, 2)
        # t=2: L-2+1 = 1 (day 2)
        return L - t + 1
    else:
        # t > L. Replenishment has arrived.
        # Need to cover demand until *next* week's replenishment.
        # Days to cover: t, t+1, ... R, 1, ... L
        # This is (R - t + 1) + L days
        # t=3: (7-3+1) + 2 = 5 + 2 = 7 days
        # t=7: (7-7+1) + 2 = 1 + 2 = 3 days
        return (R - t + 1) + L

@lru_cache(maxsize=None)
def calculate_threshold(t, mu_demand):
    """
    Calculates the threshold inventory w_j(t) to hold back.
    Based on Eq. 13 [cite: 392] and its description[cite: 390, 395, 396].
    
    Note: We use the *description* ("demand... up until replenishment" [cite: 395])
    as the literal formula `((t-L) mod R)` [cite: 392] appears inconsistent
    with the problem description (e.g., gives 0 days holding for t=2).
    """
    days_to_cover = get_days_until_replenishment(t, L_LEAD_TIME_DAYS, R_PERIOD_DAYS)
    
    if days_to_cover <= 0:
        return 0
        
    # Demand from t up until replenishment [cite: 395]
    mu_future_demand = mu_demand * days_to_cover
    
    # "maximum holding cost" [cite: 396]
    holding_cost_future = C_HOLDING * days_to_cover
    
    # Target probability from Eq. 13 [cite: 392]
    target_prob = P_PRICE / (holding_cost_future + P_PRICE)
    
    # Inverse of the cumulative distribution [cite: 395]
    threshold = poisson.ppf(target_prob, mu_future_demand)
    return int(threshold)

def tmfp_fulfillment_policy(stores, F_online_orders, t):
    """
    Allocates online orders based on the Threshold-based Myopic Policy (TMFP).
    This is identical to MFP, but respects an inventory threshold w_j(t)
    by adjusting the action space. [cite: 387, 398]
    """
    current_inventories = {s.id: s.inventory for s in stores}
    allocations = {s.id: 0 for s in stores}
    
    # Calculate thresholds for each store type
    thresholds = {}
    for s in stores:
        if s.mu_demand not in thresholds:
            thresholds[s.mu_demand] = calculate_threshold(t, s.mu_demand)
    
    for _ in range(F_online_orders):
        best_store_id = None
        best_marginal_gain = -float('inf')
        
        for s in stores:
            inv = current_inventories[s.id]
            threshold = thresholds[s.mu_demand]
            
            # This is the TMFP constraint: Eq. 14 [cite: 398]
            # We can only fulfill if inventory is *above* the threshold.
            # (I_j' - w_j(t)) > 0
            if inv > threshold:
                ec_with_fulfill = calculate_expected_contribution_next_day(inv - 1, s.mu_demand)
                ec_without_fulfill = calculate_expected_contribution_next_day(inv, s.mu_demand)
                
                marginal_oc = ec_with_fulfill - ec_without_fulfill
                gain = (P_PRICE - C_SHIPPING) + C_PENALTY + marginal_oc
                
                if gain > best_marginal_gain:
                    best_marginal_gain = gain
                    best_store_id = s.id
        
        if best_marginal_gain > 0:
            allocations[best_store_id] += 1
            current_inventories[best_store_id] -= 1
        else:
            break
            
    return allocations

# --- Policy 3: One-Step Policy Improvement (OSIP) ---

@lru_cache(maxsize=None)
def dummy_value_function_vj(inventory, mu_demand, t, q):
    """
    *** PLACEHOLDER VALUE FUNCTION ***
    The real OSIP policy requires a pre-computed value function v_n,j(s_j)
    obtained from solving the decomposed MDP with value iteration[cite: 281, 284].
    This is a highly complex calculation (Algorithm 1).
    
    For demonstration, we use a simple proxy: the value of a state is
    the expected contribution for the *rest of the period*.
    A better proxy would be needed for real results.
    """
    days_left_in_period = R_PERIOD_DAYS - t + 1
    # Simple proxy: expected profit from in-store sales for rest of period
    val = calculate_expected_contribution_next_day(inventory, mu_demand)
    return val * days_left_in_period

def osip_fulfillment_policy(stores, F_online_orders, t, value_function):
    """
    Allocates online orders based on the One-Step Policy Improvement (OSIP).
    This function implements the argmax in Eq. 10:
    
    argmax_f { C(s', f) + ~v(s'') }
    
    where:
    C(s', f) = (p-c_s) * sum(f_j) - c_l * (F - sum(f_j)) [cite: 258]
    ~v(s'') = sum_j v_j(s_j'') [cite: 305]
    
    This is a complex optimization problem. We solve it using
    dynamic programming (a multi-dimensional knapsack problem).
    
    `value_function` is the pre-computed v_n,j [cite: 284]
    """
    
    # --- 1. Define the DP state and memoization ---
    # `dp_cache[k][f]` = (max_value, allocation_list)
    # k = index of store we are considering (from 0 to len(stores)-1)
    # f = number of online orders remaining to be allocated
    dp_cache = {}

    def solve_dp(store_idx, orders_left):
        """
        Returns (max_future_value, store_allocations) for allocating
        `orders_left` among `stores[store_idx:]`.
        
        This DP solves: argmax_{f_k...f_J} sum_{j=k..J} [ (p-c_s)*f_j + v_j(I_j' - f_j) ]
        """
        if store_idx == len(stores) or orders_left == 0:
            return (0.0, [])
        
        state = (store_idx, orders_left)
        if state in dp_cache:
            return dp_cache[state]

        s = stores[store_idx]
        
        best_value = -float('inf')
        best_allocation = []
        
        # Max orders we can assign to this store: min(orders_left, I_j')
        max_f_j = min(orders_left, s.inventory)
        
        # Iterate over all possible allocations f_j for this store (from 0 to max_f_j)
        for f_j in range(max_f_j + 1):
            # C(s', f_j) for this store
            immediate_profit_fj = (P_PRICE - C_SHIPPING) * f_j
            
            # v_j(s_j''). s_j'' is the state *after* fulfillment [cite: 244, 310]
            # We need to find the state of s_j at t+1.
            # I_j'' = I_j' - f_j + (replenishment, if any)
            # Q_j'' = (new outstanding order, if any)
            
            # Check for replenishment (Eq. 4) [cite: 244]
            # Replenishment arrives at the *end* of sub-period t=L
            replenishment = s.outstanding_order if t == L_LEAD_TIME_DAYS else 0
            
            # We don't know Q_j'' without the full MDP, so we pass
            # the current Q_j as a proxy.
            future_inv = s.inventory - f_j + replenishment
            future_value_vj = value_function(future_inv, s.mu_demand, (t % R_PERIOD_DAYS) + 1, s.outstanding_order)

            # Get value from remaining stores
            value_from_rest, alloc_from_rest = solve_dp(store_idx + 1, orders_left - f_j)
            
            current_total_value = immediate_profit_fj + future_value_vj + value_from_rest
            
            if current_total_value > best_value:
                best_value = current_total_value
                best_allocation = [f_j] + alloc_from_rest
        
        dp_cache[state] = (best_value, best_allocation)
        return best_value, best_allocation

    # --- 2. Solve the full problem (Eq. 10) ---
    # We need to find the optimal *total* number of orders to fulfill,
    # from 0 to F_online_orders.
    
    best_total_profit = -float('inf')
    final_allocations = {}
    
    for F_to_fulfill in range(F_online_orders + 1):
        # C(s', f) for unfulfilled orders
        penalty_cost = C_PENALTY * (F_online_orders - F_to_fulfill)
        
        # ~v(s'') + C(s',f) for fulfilled orders
        # Note: The DP state value is (immediate_profit + future_value).
        # We need to re-run the DP for each F_to_fulfill
        dp_cache = {}
        total_value_from_dp, alloc_list = solve_dp(0, F_to_fulfill)
        
        # Total profit = DP_value - penalty
        # The v_j(s_j) term for stores that *didn't* fulfill (f_j=0)
        # is already in total_value_from_dp (it's the f_j=0 case).
        total_profit = total_value_from_dp - penalty_cost
        
        if total_profit > best_total_profit:
            best_total_profit = total_profit
            # Pad the allocation list with zeros if it's shorter than the number of stores
            padded_alloc = alloc_list + [0] * (len(stores) - len(alloc_list))
            final_allocations = {stores[i].id: padded_alloc[i] for i in range(len(stores))}

    return final_allocations


# --- Simulation Harness ---

def run_simulation():
    """
    Runs a simple simulation for one R-day period to show
    the policies in action.
    """
    print("--- 🚚 Omni-Channel Simulation Start ---")
    print(f"Base Parameters: p={P_PRICE}, c_h={C_HOLDING}, c_s={C_SHIPPING}, c_l={C_PENALTY}")
    print(f"Time: R={R_PERIOD_DAYS} days, L={L_LEAD_TIME_DAYS} days")
    
    # Create stores based on base test case [cite: 412]
    # m1=10 small, m2=10 medium, m3=10 large
    # For a simpler demo, we'll use 1 of each.
    store_list = [
        Store(id=1, type="small", mu_demand=2.0, initial_inventory=20),
        Store(id=2, type="medium", mu_demand=4.0, initial_inventory=30),
        Store(id=3, type="large", mu_demand=6.0, initial_inventory=40)
    ]
    print("\nInitial State:")
    for s in store_list: print(s)

    # Simulate outstanding orders (Q_j)
    # Assume orders were placed at t=1, to arrive at t=3 [cite: 410]
    # This is Q_j from Eq. 2 [cite: 232]
    store_list[0].outstanding_order = 15
    store_list[1].outstanding_order = 25
    store_list[2].outstanding_order = 35
    
    # Set a fixed online demand for the demo
    np.random.seed(42)
    
    # --- Loop through the 7-day period (R=7 sub-periods) ---
    for t in range(1, R_PERIOD_DAYS + 1):
        print(f"\n--- Day t={t} (Epoch 1) ---")
        
        # 1. Ordering decision (q_j)
        # This happens at t=1[cite: 161]. We are using pre-set orders.
        if t == 1:
            print("Ordering decision made (using preset Q).")
            for s in store_list:
                print(f"  {s.id}: Q_{s.id} = {s.outstanding_order}")

        # 2. In-store demand (d_j) realizes and is fulfilled [cite: 162]
        print("In-store demand occurs:")
        for s in store_list:
            demand = poisson.rvs(s.mu_demand)
            sold = min(s.inventory, demand)
            s.inventory -= sold # This is I_j' [cite: 226]
            print(f"  {s.id}: Demand={demand}, Sold={sold}, Inv={s.inventory}")

        # 3. Online orders (F) are known [cite: 181]
        F_online_orders = poisson.rvs(10) # Demo value
        print(f"Online orders received: F = {F_online_orders}")

        print(f"\n--- Day t={t} (Epoch 2) ---")
        print("Making online fulfillment decisions...")

        # Create copies of stores for each policy to make decisions on
        stores_mfp = [Store(s.id, s.type, s.mu_demand, s.inventory) for s in store_list]
        stores_tmfp = [Store(s.id, s.type, s.mu_demand, s.inventory) for s in store_list]
        stores_osip = [Store(s.id, s.type, s.mu_demand, s.inventory) for s in store_list]
        for i in range(len(store_list)):
            stores_osip[i].outstanding_order = store_list[i].outstanding_order

        # 4. Run policies to get allocation decisions (f_j)
        alloc_mfp = mfp_fulfillment_policy(stores_mfp, F_online_orders)
        alloc_tmfp = tmfp_fulfillment_policy(stores_tmfp, F_online_orders, t)
        alloc_osip = osip_fulfillment_policy(stores_osip, F_online_orders, t, dummy_value_function_vj)
        
        print(f"  MFP Policy[cite: 357]:  {alloc_mfp}")
        print(f"  TMFP Policy[cite: 389]: {alloc_tmfp} (Thresholds: {[calculate_threshold(t, s.mu_demand) for s in store_list]})")
        print(f"  OSIP Policy[cite: 275]:  {alloc_osip} (Using dummy v_j)")

        # 5. Apply one policy (e.g., OSIP) to update state for next sub-period
        # This is the state transition s' -> s'' [cite: 188]
        print("Applying OSIP policy for state transition...")
        for s in store_list:
            f_j = alloc_osip[s.id]
            s.inventory -= f_j # I_j' - f_j [cite: 244]
            
            # Check for replenishment (Eq. 4) [cite: 244]
            # `delta(t=L)`: Replenishment arrives at END of t=L.
            if t == L_LEAD_TIME_DAYS:
                print(f"  Store {s.id}: Replenishment {s.outstanding_order} arrived!")
                s.inventory += s.outstanding_order
                s.outstanding_order = 0 # Q_j'' = 0 [cite: 232, 246]
        
        print("End of day state (I_j''):")
        for s in store_list: print(s)

    print("\n--- 🚚 Simulation End ---")

if __name__ == "__main__":
    run_simulation()